...
ggplot(data=df, aes(x=dataTime,y=seoul,fill = dataTime)) +
  ...
  theme(axis.text.x=element_text(angle=90)) +
  labs(title="시간대별 서울지역의 미세먼지 농도 변화",
       x="측정일시",y="농도")+scale_fill_manual(values = rainbow(10))
...
ggplot(data=df, aes(x=dataTime,y=seoul,fill = dataTime)) +
  ...
  theme(axis.text.x=element_text(angle=90),legend.position = "none") +
  labs(title="시간대별 서울지역의 미세먼지 농도 변화",
       x="측정일시",y="농도")+scale_fill_manual(values = rainbow(10)) + coord_flip()
...
api_key <- "1F%2Buq%2B6f70YDxJUWLU7Uk9b0xw%2FA4ov0f1b%2FzKkLhwmSrWxBl9HeQplaEtnJTQC49G%2Bo05QzpfU4kTDF1cB%2Beg%3D%3D"
